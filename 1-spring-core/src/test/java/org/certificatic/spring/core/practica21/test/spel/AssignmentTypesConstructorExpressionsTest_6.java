...
public class AssignmentTypesConstructorExpressionsTest_6 {
	...
	public void assignmentExpressionsTest() {
		...
		spelParser.parseExpression("name").setValue(teslaContext, "Nikolai"); // s�lo analizar
	...
	public void typesExpressionsTest() {
		...
		// definir una expresión que calcule el área de un circulo de 5.5 de radio 
		// (multiplicar y dividir por 1000.0 para truncar decimales
		Double circleArea = spelParser.parseExpression(
				" T(Math).round( T(Math).PI * T(Math).pow('5.5', '2') * 1000.0 ) / 1000.0 ").getValue(Double.class);
	...
	public void contructorExpressionsTest() {
		...
		Inventor inventor = spelParser.parseExpression("new org.certificatic.spring.core.practica21.spel.model.Inventor("
				+ "'Albert Einstein', #birthday.getTime(), 'German')").getValue(context
				, Inventor.class);
		...
		String[] stringArray = spelParser.parseExpression(
				"new String[] {'absorption refrigerator', 'quantum theory', "
